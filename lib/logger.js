...
function sendEntry(severity, client, bucketId, ttlEntry, ttlMeta, args) {
	...
	for(let arg of args) {
		...
		if(typeof arg === 'object') {
			...
			else if(arg._metricKey && arg._metricValue && !isNaN(arg._metricValue) && arg._metricValue >= -2147483648 && arg._metricValue <= 2147483647) {
				entry.metricKeys.push(truncate(arg._metricKey, maxMetaKeySize));
				entry.metricValues.push(entry.metaValues);
			}
			else {
...
function stringify(val) {
	...
	if(typeof val === 'object') {
		if(!Array.isArray(val) && val.toString !== Object.prototype.toString) {
			return val.toString();
		}
